...
import 'package:edumate/domain/models/response/response_post.dart';
import 'package:edumate/domain/models/response/response_post_by_user.dart';
import 'package:edumate/domain/models/response/response_post_profile.dart';
import 'package:edumate/domain/models/response/response_post_saved.dart';

class PostServices {
  Future<DefaultResponse> addNewPost(
      String description, String typePrivacy, List<File> images) async {
  ...
  Future<List<Post>> getAllPostHome() async {
    ...
    final resp = await http.get(
        Uri.parse('${Environment.urlApi}/material/getall'),
        headers: {'Accept': 'application/json', 'xxx-token': token!});

    return ResponsePost.fromJson(jsonDecode(resp.body)).data;
  }
  ...
  Future<DefaultResponse> unSavePostByUser(String uidPostSave) async {
    final token = await secureStorage.readToken();

    final resp = await http.post(
        Uri.parse('${Environment.urlApi}/post/un-save-post-by-user'),
        headers: {'Accept': 'application/json', 'xxx-token': token!},
        body: {'post_save_ui': uidPostSave});
