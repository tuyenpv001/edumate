...
export const createUser = async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
        ...
        // await sendEmailVerify('message', email, `<h1> PhuotNha </h1><hr> <b>${ randomNumber } </b>`);
...
export const getProfileUser = async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
        ...
        // Lấy thông tin từ bảng user và person
        const [rows] = await conn.query(
            ...
            SELECT 
              ...
              p.birthday_date
            FROM  person p
            LEFT JOIN users u ON u.person_uid = p.uid
            WHERE p.uid = ?
            `,
            [req.idPerson]
        );
        ...
        return res.json({
            ...
            data: (rows as any).length > 0 ? (rows as any)[0] : {},
        });
    } catch (error) {
